...
function AddBrand({ getBrands }) {
  const [{ cleanString, product_category }, dispatch] = useStateValue();
  ...
  async function insertBrand(data) {
    try {
      const response = await axios({
        method: "post",
        url: "http://localhost:5000/insert-brand",
        data: data,
  ...
  const handleSubmit = async (event) => {
    ...
    if (!selectedFile) {
      setErrors({ ...errors, image: "Vui lòng chọn logo" });
    } else if (selectedBrand.trim().length == 0) {
      setErrors({ ...errors, brand: "Vui lòng nhập tên hãng" });
    } else {
      const data = {
        brand_id: cleanString(selectedBrand) + "-" + new Date().getTime(),
